            ...
            <ContextMenuTrigger>
                ...
                <div className={`flex group rounded-3xl ${selectedNodeId === props.id ? '' : ''} ${selectedNodeHistoricalNodeIds?.includes(props.id) ? 'ring-5 ring-[#88EAC9] ring-offset-1' : ''}`}>
                    ...
                    <div className={`p-4 bg-card rounded-3xl min-w-2xl ${selectedNodeHistoricalNodeIds?.includes(props.id) ? 'outline-transparent' : 'outline-outline-subtle'} max-w-2xl relative outline-2 hover:shadow-2xl duration-150 cursor-default`}>
                        {/* selected context */}
                        {nodeChat?.source && (
                            <div className='bg-muted-overlay px-4 py-3 rounded-[14px] mb-4 relative left-[-8px] top-[-8px] w-[calc(100%+16px)] flex items-center gap-2 text-sm' >
                        ...
                        <div className='flex flex-col gap-1'>
                            ...
                            <div className='flex items-center gap-2'>
                                <button className='drag-handle__ChatNode hover:bg-muted w-6 h-8 flex items-center justify-center rounded-md active:cursor-grabbing hover:cursor-grab'>
                            ...
                            {!submitted ? (
                                <textarea
                                    id=""
                                    name=""
                                    className='resize-none bg-transparent ml-5 p-3 focus:outline-none dark:text-white'
